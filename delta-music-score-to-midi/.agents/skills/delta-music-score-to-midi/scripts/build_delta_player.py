...
Run with --dry-run first. Real input additionally requires --play and typing PLAY.
F10 stops playback and releases all held controls.
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--play",
        action="store_true",
        help="allow the interactive real-input confirmation step",
    )
    ...
    if not args.play:
        print("Safety stop: no input was sent. Use --dry-run to inspect or --play to request real playback.")
        return 0

    try:
        confirmation = input(
            "This sends keyboard/mouse input to the current foreground window. Type PLAY to continue: "
        ).strip()
    except EOFError:
        print("Safety stop: no interactive confirmation was available; no input was sent.")
        return 2
    if confirmation != "PLAY":
        print("Cancelled: no input was sent.")
        return 0
